...
import { verification } from '~/drizzle/schema.server.ts'
		...
		schema: () =>
			verifySchema.superRefine(async (data, ctx) => {
				...
				const verificationToken = await db
					...
					.where(
						and(
							...
							eq(verification.target, data.email),
				...
				const result = verifyTOTP({
					...
					algorithm: verificationToken.algorithm,
					period: verificationToken.period,
					window: 0,
				})
				if (!result) {
					ctx.addIssue({
